...
const GRAVITY = 0.015; // Extremely slow, almost zero gravity
const TRAIL_MAX_AGE = 150; // ms
...
export default function App() {
  ...
  const spawnItem = (width: number, height: number) => {
    ...
    // Difficulty increases speed very slightly over time
    const difficulty = 1 + (score / 1000);
    const x = Math.random() * (width - 60) + 30;
    const vx = (Math.random() - 0.5) * 2 * difficulty; 
    const vy = -(Math.random() * 2 + 5) * difficulty; // Much lower base jump
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      const deltaTime = Math.min(time - lastTime.current, 32); // Cap delta time to avoid huge jumps
      ...
      // Clear canvas to show CSS gradient background
      ...
      if (spawnTimer.current > spawnRate) {
        const count = Math.floor(Math.random() * 3) + 3; // Spawn 3-5 items at once
  ...
  return (
    <div className="w-full h-screen bg-slate-950 flex flex-col items-center justify-start p-0">
      <div 
        ref={containerRef}
        className="relative w-full h-[50vh] bg-gradient-to-br from-indigo-900 via-purple-900 to-rose-900 overflow-hidden font-sans select-none touch-none border-b-4 border-white/10 shadow-2xl"
      >
        {/* Background Decor */}
        ...
        <AnimatePresence>
          {gameState === 'START' && (
            ...
            >
              <div className="absolute inset-0 opacity-10 pointer-events-none overflow-hidden">
                <div className="absolute top-1/4 left-1/2 -translate-x-1/2 text-8xl">🥖</div>
                <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 text-8xl">🍣</div>
              </div>
          ...
          {gameState === 'GAMEOVER' && (
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-xl p-10 rounded-[3rem] mb-10 border-2 border-white/20 w-full shadow-2xl">
